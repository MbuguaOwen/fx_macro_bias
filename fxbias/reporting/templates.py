...
<div class="card" style="margin-top:14px;"><h2>Latest Week Leaderboard ($LATEST_WEEK)</h2><div class="table-wrap"><table class="tbl"><thead><tr><th>Pair</th><th>Rates</th><th>Growth</th><th>Risk</th><th>Positioning</th><th>Total</th><th>Conviction</th><th>Bias</th><th>Staleness</th></tr></thead><tbody>$LATEST_ROWS</tbody></table></div></div>
<div class="grid two" style="margin-top:14px;">
...
<div class="card"><h2>Latest Week Sentiment Table</h2><div class="table-wrap"><table class="tbl" id="tbl-sentiment-latest"></table></div></div>
</div>
...
const DATA=$PAYLOAD_JSON,weeks=DATA.weeks||[],pairs=DATA.pairs||[],rows=DATA.rows||[],scoreCol=DATA.score_col||"total_score",marketOverlay=DATA.market_overlay||{},marketStatus=marketOverlay.status||{},marketReq=marketOverlay.request||{},sentimentOverlay=DATA.sentiment_overlay||{},sentimentStatus=sentimentOverlay.status||{},sentimentSummary=sentimentOverlay.summary||{};
...
const pairSel=document.getElementById("sel-pair"),weekSel=document.getElementById("sel-week"),sp=document.getElementById("sentiment-pair"),sw=document.getElementById("sentiment-week"),defaultPair=pairs[0]||"",defaultWeek=weeks[weeks.length-1]||"";
fill(pairSel,pairs);fill(sp,pairs);fill(weekSel,weeks);fill(sw,weeks);
function setSelection(pair,week){const p=pairs.includes(pair)?pair:defaultPair,w=weeks.includes(week)?week:defaultWeek;pairSel.value=p;weekSel.value=w;sp.value=p;sw.value=w;updateDrill();updateSentiment()}
pairSel.addEventListener("change",()=>setSelection(pairSel.value,weekSel.value));weekSel.addEventListener("change",()=>setSelection(pairSel.value,weekSel.value));sp.addEventListener("change",()=>setSelection(sp.value,sw.value));sw.addEventListener("change",()=>setSelection(sp.value,sw.value));
...
function renderOverview(){
 const k=DATA.overview_kpis||{};document.getElementById("kpi-pairs").textContent=k.pairs_tracked??"NA";document.getElementById("kpi-bull").textContent=k.macro_bullish??"NA";document.getElementById("kpi-bear").textContent=k.macro_bearish??"NA";document.getElementById("kpi-neut").textContent=k.macro_neutral??"NA";document.getElementById("kpi-cover").textContent=k.sentiment_coverage??"NA";document.getElementById("kpi-agree").textContent=pct(k.sentiment_agreement_rate);
 document.getElementById("exec-macro").textContent=`${k.pairs_tracked||0} pairs tracked for ${DATA.latest_week||"NA"} with ${k.macro_bullish||0} bullish, ${k.macro_bearish||0} bearish, and ${k.macro_neutral||0} neutral macro calls.`;
 document.getElementById("exec-sentiment").textContent=!sentimentOverlay.requested?"Sentiment overlay was not requested for this run.":`Coverage ${sentimentSummary.coverage_count||0} | bullish ${(sentimentSummary.bias_counts||{}).BULLISH||0} | bearish ${(sentimentSummary.bias_counts||{}).BEARISH||0} | agreement ${pct(sentimentSummary.agreement_rate)}.`;
 document.getElementById("exec-scope").textContent=`Core engine: rates, growth, risk, positioning. Overlay layers stay informational only and never rewrite total score or final bias.`;
 const ovLatest=marketOverlay.latest||null,ovNode=document.getElementById("overlay-summary"),ovTitle=document.getElementById("overlay-title");ovTitle.textContent=`Market Overlay Summary (${String((ovLatest&&ovLatest.symbol)||marketReq.symbol||"XAUUSD").toUpperCase()})`;ovNode.innerHTML=!ovLatest?`${marketStatus.message||"Overlay unavailable."}${marketStatus.hint?`<br/>${marketStatus.hint}`:""}`:`${sentPill(ovLatest.label||"NEUTRAL")} rr10=${fmt(ovLatest.rr10,2)} | rr25=${fmt(ovLatest.rr25,2)} | atm_iv=${fmt(ovLatest.approx_atm_iv,2)} | tenor=${ovLatest.tenor||"NA"} | expiry=${ovLatest.expiry_date||"NA"} | as_of=${ovLatest.as_of||"NA"}`;
 const sNode=document.getElementById("sentiment-overview");sNode.innerHTML=!sentimentOverlay.requested?`${sentimentStatus.message||"Sentiment overlay not requested."}${sentimentStatus.hint?`<br/>${sentimentStatus.hint}`:""}`:!(sentimentOverlay.entries||[]).length?`${sentimentStatus.message||"Sentiment overlay unavailable."}${sentimentStatus.hint?`<br/>${sentimentStatus.hint}`:""}`:`Coverage=${sentimentSummary.coverage_count||0} | Bullish=${(sentimentSummary.bias_counts||{}).BULLISH||0} | Bearish=${(sentimentSummary.bias_counts||{}).BEARISH||0} | Neutral=${(sentimentSummary.bias_counts||{}).NEUTRAL||0} | Agreement=${pct(sentimentSummary.agreement_rate)} | Stale=${sentimentSummary.stale_count||0}`;
...
function updateSentiment(){
 const pair=String(sp.value||"").toUpperCase(),week=String(sw.value||""),row=idx.get(key(pair,week)),sent=sentimentFor(pair,week);document.getElementById("sentiment-selection-meta").textContent=`ID=${key(pair,week)}`;
 ...
 const latestRows=(sentimentSummary.latest_rows||[]).map(item=>{const macro=idx.get(key(item.symbol,item.as_of));return `<tr class="row-click" data-pair="${item.symbol}" data-week="${item.as_of}" data-tab="sentiment"><td>${item.symbol}</td><td>${macro?macroPill(macro.final_bias||macro.bias||"NEUTRAL"):"NA"}</td><td>${sentPill(item.sentiment_bias)}</td><td>${fmt(item.sentiment_score,3)}</td><td>${item.sentiment_conviction||"NA"}</td><td>${agreePill(item.agreement_with_macro)}</td><td>${freshPill(item.sentiment_stale)}</td><td class="wrap">${item.headline_summary||""}</td></tr>`}).join("");document.getElementById("tbl-sentiment-latest").innerHTML=`<thead><tr><th>Pair</th><th>Macro Bias</th><th>Sentiment Bias</th><th>Score</th><th>Conviction</th><th>Agreement</th><th>Freshness</th><th>Summary</th></tr></thead><tbody>${latestRows}</tbody>`;for(const tr of document.querySelectorAll('#tbl-sentiment-latest tr.row-click'))tr.addEventListener("click",()=>jump(tr.dataset.pair,tr.dataset.week,tr.dataset.tab||"sentiment"));
}
...
for(const tr of document.querySelectorAll("tr.row-click"))tr.addEventListener("click",()=>jump(tr.dataset.pair,tr.dataset.week,tr.dataset.tab||"drill"));
